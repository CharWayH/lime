package com.charwayh.lime.support.mq.rabbit;
...
@Slf4j
@Service
@ConditionalOnProperty(name = "lime.mq.pipeline", havingValue = MessageQueuePipeline.RABBIT_MQ)
public class RabbitSendMqServiceImpl implements SendMqService {

    @Autowired
    private RabbitTemplate rabbitTemplate;

    @Value("${lime.rabbitmq.topic.name}")
    private String confTopic;

    @Value("${lime.rabbitmq.exchange.name}")
