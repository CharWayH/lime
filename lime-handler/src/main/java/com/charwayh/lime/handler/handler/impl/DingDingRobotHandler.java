...
import com.charwayh.lime.common.constant.SendAccountConstant;
import com.charwayh.lime.common.domain.TaskInfo;
import com.charwayh.lime.common.dto.account.DingDingRobotAccount;
import com.charwayh.lime.common.dto.model.DingDingRobotContentModel;
import com.charwayh.lime.common.enums.ChannelType;
import com.charwayh.lime.common.enums.SendMessageType;
import com.charwayh.lime.handler.domain.dingding.DingDingRobotParam;
import com.charwayh.lime.handler.domain.dingding.DingDingRobotResult;
...
import com.charwayh.lime.support.utils.AccountUtils;
...
public class DingDingRobotHandler extends BaseHandler implements Handler {
    ...
    private DingDingRobotParam assembleParam(TaskInfo taskInfo) {
        ...
        if (LimeConstant.SEND_ALL.equals(CollUtil.getFirst(taskInfo.getReceiver()))) {
    ...
    private String assembleSign(long currentTimeMillis, String secret) {
        ...
        try {
            ...
            Mac mac = Mac.getInstance(LimeConstant.HMAC_SHA256_ENCRYPTION_ALGO);
            mac.init(new SecretKeySpec(secret.getBytes(LimeConstant.CHARSET_NAME), LimeConstant.HMAC_SHA256_ENCRYPTION_ALGO));
            byte[] signData = mac.doFinal(stringToSign.getBytes(LimeConstant.CHARSET_NAME));
            sign = URLEncoder.encode(new String(Base64.encodeBase64(signData)), LimeConstant.CHARSET_NAME);
        } catch (Exception e) {
